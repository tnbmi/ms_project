...
bool EffectManager::Create(EffectManager** outPointer,LPDIRECT3DDEVICE9 device,ObjectList* objectList,UpdateList *updateList,DrawListManager *drawList,
							const int PolygonMax,const char *texFilePath,const D3DXVECTOR2 &texSize,const D3DXVECTOR2 &oneSize )
{
	...
	EffectManager *ef = new EffectManager( bill );
	if( !ef->Initialize() )
		return false;
