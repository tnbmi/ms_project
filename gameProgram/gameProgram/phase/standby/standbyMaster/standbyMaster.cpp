...
bool StandbyMaster::Initialize(void)
{
	...
	//スタンバイ
	m_blueTeamStandby[0].pol->scl( 0,0,1 );
	m_blueTeamStandby[1].pol->scl( 0,0,1 );

	m_redTeamStandby[0].pol->scl( 0,0,1 );
	m_redTeamStandby[1].pol->scl( 0,0,1 );

	//235  593
	//452  593
	//832  593
	//1059 593
	m_blueTeamStandby[0].pol->pos( 235,593,0 );
	m_blueTeamStandby[1].pol->pos( 452,593,0 );
	m_redTeamStandby[0].pol->pos( 832,593,0 );
	m_redTeamStandby[1].pol->pos( 1059,593,0 );
	...
	//ロゴ
	m_blueLogo.pol->pos( 340,689,0);
	m_blueLogo.pol->scl(417,59,1 );
	m_blueLogo.pol->texcoord(0,0,0);
	m_blueLogo.pol->texcoord(1,1,0.111);
	m_blueLogo.pol->texcoord(2,0,0);
	m_blueLogo.pol->texcoord(3,1,0.111);
	m_blueLogo.uv[0] = D3DXVECTOR2(0,0);
	m_blueLogo.uv[1] = D3DXVECTOR2(1,0);
	m_blueLogo.uv[2] = D3DXVECTOR2(0,0.111);
	m_blueLogo.uv[3] = D3DXVECTOR2(1,0.111);
	m_blueLogo.offsetUv = D3DXVECTOR2(0,0.111);
	m_blueLogo.animSum = 9;
	m_blueLogo.time = 0;

	m_redLogo.pol->pos(944,689,0);
	m_redLogo.pol->scl(417,59,1);
	m_redLogo.uv[0] = D3DXVECTOR2(0,0.999);
	m_redLogo.uv[1] = D3DXVECTOR2(1,0.999);
	m_redLogo.uv[2] = D3DXVECTOR2(0,0.999);
	m_redLogo.uv[3] = D3DXVECTOR2(1,1);
	m_redLogo.offsetUv = D3DXVECTOR2(0,0.111);
...
void StandbyMaster::Update(void)
{
	...
	D3DXVECTOR2 offset = m_blueLogo.offsetUv * animNum;
	...
	case PHASE_WAIT:
		...
		for( int i = 0 ; i < 2 ; i++ )
		{
			...
			{
				D3DXVECTOR3 scl;
				scl = LerpVec3( D3DXVECTOR3(0,0,0),D3DXVECTOR3( 91/2,269/2,1 ),0,_compFrame,m_blueTeamStandby[i].time,Cube );
			...
			{
				D3DXVECTOR3 scl;
				scl = LerpVec3( D3DXVECTOR3( 91/2,269/2,1 ),D3DXVECTOR3( 0,0,0 ),0,_compFrame,m_blueTeamStandby[i].time,Cube );
				m_blueTeamStandby[i].pol->scl(scl);
		...
		for( int i = 0 ; i < 2 ; i++ )
		{
			...
			{
				D3DXVECTOR3 scl;
				scl = LerpVec3( D3DXVECTOR3(0,0,0),D3DXVECTOR3( 91/2,269/2,1 ),0,_compFrame,m_redTeamStandby[i].time,Cube );
			...
			{
				D3DXVECTOR3 scl;
				scl = LerpVec3( D3DXVECTOR3( 91/2,269/2,1 ),D3DXVECTOR3( 0,0,0 ),0,_compFrame,m_redTeamStandby[i].time,Cube );
				m_redTeamStandby[i].pol->scl(scl);
	...
	case PHASE_STANDBY:
		...
		battleScl = LerpVec3( D3DXVECTOR3( 781*4,413*4,1 ),D3DXVECTOR3( 781/2,413/2,0 ),0,_compFrame,m_compTime,Cube );
