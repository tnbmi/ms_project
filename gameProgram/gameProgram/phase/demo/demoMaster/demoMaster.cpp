...
// 初期化
//=============================================================================
bool DemoMaster::Initialize(void)
{

	//コマンドマネージャ
	CommandManager::Create(&m_command_manager, m_padXManager, m_debugProc, m_objectList, m_updateList, m_drawListManager, m_device, m_import, true);
	// タイムマネージャ生成
	...
	//カットイン
	//----------------------------
	Polygon2D::Create( &m_redTeamCutIn.pol,m_device,m_objectList,m_import->texture(DemoImport::CUTINRED),ObjectBase::TYPE_2D );
	Polygon2D::Create( &m_blueTeamCutIn.pol,m_device,m_objectList,m_import->texture(DemoImport::CUTINBLUE),ObjectBase::TYPE_2D );
	...
	m_redTeamCutIn.pol->pos(SCREEN_WIDTH + 200.0f,500.0f,0.0f);
	...
	m_blueTeamCutIn.pol->pos(-200.0f,500.0f,0.0f);
...
void DemoMaster::AddTeamScore( const int addRedTeamVal,const int addBlueTeamVal )
{
...
void DemoMaster::DetermineTeamScore()
{
	int rOffset = 0;
	int bOffset = 0;
...
//カウントダウン更新
//=============================================================================
void DemoMaster::UpdateCountDown()
{
	m_call->scl( 400,200,0 );
	m_countDown->Update();
	if( m_countDown->isEndAnimation() )
	{
		m_demoPhase = PHASE_CALL;
...
//移行更新
//=============================================================================
bool DemoMaster::UpdateNextScene()
{
	m_callTime++;
